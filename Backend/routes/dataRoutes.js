...
router.get('/events', async (req, res) => {
    try {
        ...
        }).select('-draftChanges').sort({ createdAt: -1 });
        res.json({
            success: true,
            events
        });
...
// GET all contests (Public - Approved only)
...
router.get('/contests/:id', async (req, res) => {
    try {
        ...
        const contest = await Contest.findOne({
            _id: contestId,
            $or: [{ status: 'approved' }, { status: { $exists: false } }]
        }).select('-draftChanges').populate('participants', 'name email');
...
// GET all news (Public - Approved only)
